        ...
        for i, url in enumerate(urls):
            ...
            try:
                if "linkedin.com" in url:
                    # Use LinkedIn-specific scraper (best-effort)
                    li_results = self.linkedin_scraper.scrape_posts([url])
                    if li_results:
                        scraped_articles.extend(li_results)
                        st.success(f"✅ Successfully scraped LinkedIn: {url}")
                    else:
                        st.warning(f"⚠️ No usable LinkedIn content found: {url}")
                else:
                    # Download and extract content via trafilatura
